<!-- docs/_sidebar.md -->
* [首页](/)
* [前言和法律声明](docs/前言和法律声明.md)
* [2_安装和升级MySQL](docs/ch2/2_安装和升级MySQL.md)
  * [2.1_安装指引说明](docs/ch2/2.1_安装指引说明.md)
    * [2.1.1_需要什么版本的MySQL](docs/ch2/2.1.1_需要什么版本的MySQL.md)
    * [2.1.2_如何获取MySQL](docs/ch2/2.1.2_如何获取MySQL.md)
    * [2.1.3_使用MD5或GnuPG验证安装包](docs/ch2/2.1.3_使用MD5或GnuPG验证安装包.md)
      * [2.1.3.1_使用MD5验证安装包](docs/ch2/2.1.3.1_使用MD5验证安装包.md)
      * [2.1.3.2_使用GunPG验证签名](docs/ch2/2.1.3.2_使用GunPG验证签名.md)
    * [2.1.4_安装布局](docs/ch2/2.1.4_安装布局.md)
* [23_分区](docs/ch23/23_分区.md)
# 第三章 使用教程
本章提供了一个建议教程来展示如何使用 [**mysql**](programs.md＃mysql) 客户端程序来创建和使用简单数据库[**mysql**](programs.md＃mysql)（有时称为“终端监视器”或简称“监视器”）是一个交互式程序，使您可以连接到MySQL服务器，运行查询并查看结果。 [**mysql**](programs.md＃mysql)也可以在批处理模式下使用：将查询事先放在文件中，然后告诉[**mysql**](programs.md＃mysql)执行文件内容。这里介绍了使用[**mysql**](programs.md＃mysql)的两种方法。

如果想查看 [**mysql**](programs.md#mysql) 提供的命令行参数,   请使用 [`--help`](programs.md#option_mysql_help) 参数:  
